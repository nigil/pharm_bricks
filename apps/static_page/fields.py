...
from wagtail.wagtailcore.blocks import (
    StructBlock, StreamBlock, CharBlock, RawHTMLBlock,
...
class BodyStreamBlock(StreamBlock):
    ...
    """
    h2 = CharBlock(icon="title", required=False)
    h3 = CharBlock(icon="title", required=False)
    paragraph = RichTextBlock(icon="pilcrow", required=False)
    image = ImageBlock(label=_("Aligned image"), icon="image", required=False)
    video = VideoBlock(label=_("Embedded video"), icon="media", required=False)
    pullquote = PullQuoteBlock(label=_("Quote"), icon="openquote", required=False),
